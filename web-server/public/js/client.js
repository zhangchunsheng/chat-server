...
function addMessage(from, scope, toName, text, time) {
	var name = (scope == SCOPE.ALL ? 'all' : toName);
	if(text === null) return;
	if(time == null) {
...
$(document).ready(function() {
	...
	pomelo.on('onChat', function(data) {
		addMessage(data.from, data.scope, data.toName, data.content);
		$("#chatHistory").show();
		...
		if(!util.isBlank(msg)) {
			pomelo.request(route, {
				...
				if(toName != '*' && toName != username) {
					addMessage(username, scope, toName, msg);
					$("#chatHistory").show();
				}
			});
		}
